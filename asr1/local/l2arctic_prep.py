...
for phn_path in wav_lst:
    ...
    if(spk_id in eval(type_[0]+"_spk") ):
        cur_phns = []
        transcript_phns = []
        #detections = [] # NOTE: detection labels
        tg = textgrid.TextGrid.fromFile(phn_path)
        for i in tg[1]:
            if(i.mark == ''):
                transcript_phns.append(("sil"))
                cur_phns.append("sil")
                #detections.append("1") # NOTE: detection labels
            else:
                ...
                if( len(trans_human_type) == 1 ):
                    ...
                    #detections.append("0") # NOTE: detection labels
        ...
        '''
        del_transcript_phns = del_repeat_sil(transcript_phns)
        del_phns = del_repeat_sil(cur_phns)
        print(utt_id)
        print(len(del_phns))
        print(" ".join(del_phns))
        print(len(del_transcript_phns))
        print(" ".join(del_transcript_phns))
        ...
        #w5.write(utt_id + " " + " ".join(detections) + "\n" ) # NOTE: detection labels
...
w3.close()
w4.close()
#w5.close() # NOTE: detection labels
